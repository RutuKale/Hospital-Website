          ...
          <div className="hidden md:flex absolute left-1/3 -translate-x-1/4 space-x-8 lg:space-x-10">
            {navLinks.map((link) => (
              ...
              >
                <Link
                  to={link.href}
                  className="relative group text-gray-700 hover:text-blue-600 px-3 py-2 rounded-md sm:text-sm lg:text-lg font-bold transition-colors flex items-center gap-1"
                >
                  {link.name}
                  {link.subLinks && (
                    <svg
