...
const doctorsList = [
    ...
    { name: "Dr. Jessica Davis", role: "Dermatologist", image: "https://images.unsplash.com/photo-1622253692010-333f2da6031d?auto=format&fit=crop&q=80&w=800" },
];
...
const FindDoctor = () => {
    ...
    const handleScroll = () => {
        if (scrollRef.current) {
            ...
            // Calculate current index based on the middle set
            // The middle set starts at index 'originalLength'
            let relativeIndex = Math.round(scrollLeft / itemWidth) - originalLength;

            // Normalize index to 0..originalLength-1
            let actualIndex = relativeIndex % originalLength;
            if (actualIndex < 0) actualIndex += originalLength;

            setActiveIndex(actualIndex);

            // Teleport logic for infinite scrolling
            // If scrolled to the first set (left buffer), jump to middle set
            ...
            // If scrolled to the third set (right buffer), jump to middle set
    ...
    const scrollToDot = (index) => {
        if (scrollRef.current) {
            // Scroll to the specific item in the MIDDLE set
            // Middle set starts at originalLength
        ...
        <section className="py-10 sm:py-12 md:py-14 lg:py-16 bg-gray-50">
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                <div data-aos="fade-up" className="text-center md:mb-12 mb-8">
                    <span className="text-blue-600 font-bold uppercase tracking-wider text-sm">Meet Our Experienced Team</span>
                    <h2 className="text-xl sm:text-2xl md:text-3xl lg:text-4xl font-extrabold text-gray-900 mt-2 md:mb-4 mb-3">Find A Doctor</h2>
                ...
                {/* Doctors Carousel */}
                <div className="relative group px-2 sm:px-6 md:px-12"
                ...
                >
                    ...
                    >
                        {extendedDoctors.map((doctor, index) => (
                            <div key={index} data-aos="zoom-in" className="flex-none w-[85%] sm:w-[45%] md:w-[280px] snap-center">
                                <div className="bg-white rounded-2xl overflow-hidden shadow-lg hover:shadow-2xl transition-all duration-300 transform hover:-translate-y-2 border border-gray-100 h-full flex flex-col">
                                    ...
                                    <div className="p-4 sm:p-5 md:p-6 text-center flex-grow flex flex-col justify-center bg-white">
                                        <h3 className="text-sm sm:text-base md:text-lg font-bold text-gray-900 mb-1">{doctor.name}</h3>
                                        <p className="text-blue-600 text-xs sm:text-sm md:text-base font-medium">{doctor.role}</p>
                                    </div>
                                </div>
                            </div>
                        ))}
